...
def _get_connection():
    ...
    # Try to connect, and if logs table is missing, run init_db
...
def log_history(entry: str):
    ...
    c = conn.cursor()
    c.execute("INSERT INTO logs (timestamp, event, log_type) VALUES (datetime('now'), ?, ?)",
              (entry, "DASHBOARD"))
    conn.commit()
    conn.close()
...
def show_history(limit: int = 50):
    ...
    conn = _get_connection()
    c = conn.cursor()
    c.execute("SELECT event, timestamp, log_type FROM logs ORDER BY id DESC LIMIT ?", (limit,))
    rows = c.fetchall()
    conn.close()

    if rows:
        st.dataframe([{"event": row[0], "timestamp": row[1], "log_type": row[2]} for row in rows])
    else:
